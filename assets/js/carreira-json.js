...
const renderProfile = ({ profile, contacts }) => {
	...
	// also populate the small icon links in the header
	const headerNav = document.querySelector("nav.secondary-info");
	if (headerNav) headerNav.innerHTML = "";

	contacts.forEach((contact) => {
		const listItem = createElement("li", "mb-2 d-flex align-items-center");
		const link = createElement("a", "text-link d-inline-flex align-items-center", contact.label);
		...
		// choose a simple emoji icon based on url
		const icon = createElement("span", "me-2");
		if (contact.url.includes("linkedin")) {
			icon.textContent = "💼";
		} else if (contact.url.includes("github")) {
			icon.textContent = "🐱";
		} else if (contact.url.includes("@") || contact.url.startsWith("mailto")) {
			icon.textContent = "✉️";
		} else {
			icon.textContent = "🔗";
		}
		...
		// header icon link (icon-only)
		if (headerNav) {
			const navLink = createElement("a", "icon-link icon-link-hover", "");
			navLink.href = contact.url;
			if (contact.url.startsWith("http")) {
				navLink.target = "_blank";
				navLink.rel = "noopener";
			}
			navLink.setAttribute("aria-label", contact.label);
			const navIcon = createElement("span", "fa-container");
			navIcon.textContent = icon.textContent;
...
const renderCareerTimeline = (careerSteps) => {
	...
	careerSteps.forEach((step, index) => {
		const revealDirection = index % 2 === 0 ? "reveal-left" : "reveal-right";
		const article = createElement("article", `resume-timeline-item position-relative pb-5 ${revealDirection}`);
...
const setupScrollReveal = () => {
	const revealTargets = document.querySelectorAll(
		".resume-section, .resume-timeline-item, .resume-header"
	);
